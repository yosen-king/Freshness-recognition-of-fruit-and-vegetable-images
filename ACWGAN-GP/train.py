...
DATA_DIR = '../archive'
# VAL_DIR = '../cuicode/archive'

IMAGE_DATA_SET = 'fruits_rotten_and_fresh' #change this to something else, e.g. 'imagenets' or 'raw' if your data is just a folder of raw images. 

#If you use lmdb, you'll need to write the loader by yourself, see load_data
TRAINING_CLASS = ['fresh_apple', 'fresh_banana', 'fresh_bitter_gourd', 'fresh_capsicum','fresh_orange','fresh_tomato','stale_apple','stale_banana','stale_bitter_gourd','stale_capsicum','stale_orange','stale_tomato']
...
OUTPUT_PATH = '/root/autodl-tmp/output/' # output path where result (.e.g drawing images, cost, chart) will be stored
# MODE = 'wgan-gp'
...
def load_data(path_to_folder, classes):
    data_transform = transforms.Compose([
                 transforms.Resize([64,64]),  #不能单写64以等宽比缩小，因为图片都不一样
    ...
    train_image_datasets, test_image_datasets = torch.utils.data.random_split(dataset,[train_size,test_size])
    
    
    dataset_loader_train = torch.utils.data.DataLoader(train_image_datasets,batch_size=BATCH_SIZE, shuffle=True, num_workers=5, drop_last=True, pin_memory=True)
    dataset_loader_valid = torch.utils.data.DataLoader(test_image_datasets,batch_size=BATCH_SIZE, shuffle=True, num_workers=5, drop_last=True, pin_memory=True)
    
    return dataset_loader_train, dataset_loader_valid
...
def generate_image(netG, noise=None):
    if noise is None:
        rand_label = np.array(np.arange(0,12).tolist()*10)   #这里生成的要比batch_size大！！！！
        rand_label = rand_label[:BATCH_SIZE]
    ...
    return samples[:60,:,:,:]   #为了输出的图好看，防止除不尽
...
def train():
    ...
    dataloader = load_data(DATA_DIR, TRAINING_CLASS)[0]
    val_loader = load_data(DATA_DIR, TRAINING_CLASS)[1]
	...
	#----------------------Generate images-----------------
        ...
        if iteration % 200==199:
            # val_loader = load_data(VAL_DIR, VAL_CLASS)[1]  
            dev_disc_costs = []
            ...
            torchvision.utils.save_image(gen_images, OUTPUT_PATH + 'samples_{}.png'.format(iteration), nrow=12, padding=2)
            grid_images = torchvision.utils.make_grid(gen_images, nrow=12, padding=2)
